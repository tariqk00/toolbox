...
def dedupe_action_items(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Deduplicate action items by normalized subject and sender."""
...
def create_unique_tasks(
    ...
    """Create only tasks that are not already open and not duplicated in the same batch."""
    from toolbox.lib.tasks import create_task
    ...
    try:
        ...
        existing_titles = {
            normalize_task_title(item.get("title", ""))
            for item in result.get("items", [])
            if item.get("title")
        }
    except Exception as e:
        logger.warning("Could not load existing Google Tasks for dedup: %s", e)
        existing_titles = set()
    ...
    for task in tasks:
        title = title_fn(task).strip()
        if not title:
            continue

        dedupe_key = key_fn(task) if key_fn else title
        normalized_key = normalize_task_title(str(dedupe_key))
        normalized_title = normalize_task_title(title)

        if normalized_key in seen_batch or normalized_title in existing_titles:
            continue
        ...
        create_task(
            ...
            list_id,
            title,
            due=due_fn(task) if due_fn else None,
            notes=notes_fn(task) if notes_fn else None,
        )
        seen_batch.add(normalized_key)
