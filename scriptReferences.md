...
| **[toolbox/bin/find_folders.py](toolbox/bin/find_folders.py)** | |
...
These servers extend Antigravity's capabilities and are configured in `~/.gemini/antigravity/mcp_config.json`.

| Server                | Transport                                       | Key Tools                                                                                         |
| :-------------------- | :---------------------------------------------- | :------------------------------------------------------------------------------------------------ |
| **github-mcp-server** | Docker (`ghcr.io/github/github-mcp-server`)     | `search_repositories`, `get_file_contents`, `push_files`, `create_pull_request`, `list_commits`   |
| **n8n-mcp-server**    | Docker (`ghcr.io/czlonkowski/n8n-mcp:latest`)   | `get_node`, `validate_workflow`, `n8n_create_workflow`, `n8n_list_workflows`, `n8n_test_workflow` |
| **docker-nuc**        | SSH (`tariqk@172.30.0.169`)                     | `list_containers`, `run_container`, `stop_container`, `fetch_container_logs`, `list_images`       |
| **google-drive**      | Python (`toolbox/mcp-servers/gdrive/server.py`) | `search` (file search by name/content)                                                            |

---

## n8n Workflows

Workflow JSON files stored in `toolbox/n8n/`. Deploy via `n8n-mcp-server` or n8n UI import.

| Workflow File                                                                        | Trigger        | Description                                                                                                   |
| :----------------------------------------------------------------------------------- | :------------- | :------------------------------------------------------------------------------------------------------------ |
| **[toolbox/n8n/readwise_daily_digest.json](toolbox/n8n/readwise_daily_digest.json)** | Cron (7:00 AM) | Fetches top 3 unread articles from Readwise Reader, summarizes via Gemini, posts digest to Google Chat Space. |
