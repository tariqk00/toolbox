...
- **Shared Runtime Venv**: `cd ~/github/tariqk00/toolbox && source google-drive/venv/bin/activate`
- **Tests**: `./google-drive/venv/bin/python3 -m pytest tests/ -q` — **MUST** pass before any commit.
- **Local Tooling Venv**: `toolbox/venv` also exists for newer local tooling; do not assume it is the shared runtime used by `setup`.
